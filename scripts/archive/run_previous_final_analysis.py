...
RANDOM_STATE = 42
OUT = Path('ecoli_amr_resultados_finales')
...
def bootstrap_metric_ci(y_true, y_pred, y_proba, metric_name, n_boot=800, seed=42):
    ...
    for _ in range(n_boot):
        ...
        if metric_name == 'accuracy':
            ...
            if len(np.unique(yt)) < 2 and np.sum(yt == 1) == 0:
                continue
...
def evaluate_predictions(y_true, y_pred, y_proba):
    ...
    for m in ['accuracy', 'balanced_accuracy', 'f1_resistant', 'roc_auc', 'recall_resistant', 'precision_resistant', 'specificity_susceptible']:
...
def plot_class_balance(antibiotic, y):
    counts = pd.Series(y).map({0: 'Susceptible', 1: 'Resistente'}).value_counts().reindex(['Susceptible', 'Resistente'])
    ...
    for bar, val in zip(bars, counts.values):
        ax.text(bar.get_x() + bar.get_width()/2, val, f'{val}\n({fmt_pct(val/total)})', ha='center', va='bottom', fontsize=9)
    fig.tight_layout()
...
def plot_model_comparison(antibiotic, model_rows):
    ...
    for bar, mean, sd in zip(bars, means, sds):
        ax.text(bar.get_x() + bar.get_width()/2, mean, f'{fmt_sig(mean)} ± {fmt_sig(sd, 2)}', ha='center', va='bottom', fontsize=9)
    fig.tight_layout()
...
def plot_confusion_matrix_es(antibiotic, model_label, cm):
    ...
    for i in range(2):
        for j in range(2):
            text = f'{cm[i, j]}\n{fmt_pct(norm[i, j])}'
            ax.text(j, i, text, ha='center', va='center', color='white' if norm[i, j] > thresh else 'black', fontsize=10)
    fig.tight_layout()
...
def plot_stable_genes(antibiotic, model_label, stability, total_folds, n=15):
    ...
    for bar, freq, frac in zip(bars, top['selection_frequency'], top['selection_fraction']):
        ax.text(bar.get_width(), bar.get_y() + bar.get_height()/2, f' {int(freq)}/{total_folds} ({fmt_pct(frac)})', va='center', fontsize=9)
    fig.tight_layout()
...
candidate_models = {
    'logistic_regression_balanced': {
        ...
        'estimator': make_pipeline(
            ...
            LogisticRegression(max_iter=4000, class_weight='balanced', solver='liblinear', random_state=RANDOM_STATE)
        ),
        'param_grid': {
...
for antibiotic, path in DATASETS.items():
    if not path.exists():
        raise FileNotFoundError(f'No encontré {path}. Pon este script en la misma carpeta que los archivos dataset_*_model_ready.csv.')
    ...
    # Selección del mejor modelo por exactitud balanceada en CV y desempate por ROC AUC en CV
    comp_df = pd.DataFrame(rows_antibiotic).sort_values(
    ...
    holdout_selected['selection_rule'] = 'Mayor exactitud balanceada en validación cruzada interna (5 folds × 2 repeticiones)'
    ...
    # Ajustar nuevamente el modelo seleccionado para figuras finales e interpretación
    ...
    # Importancia y estabilidad sobre el dataset completo, luego de elegir la familia del modelo
    ...
    raw, stability, total_folds = summarize_gene_stability(final_all_data, selected_family, X, y, feature_cols, antibiotic)
...
for _, row in selected_df.iterrows():
    ...
    selected_sections.append(
        ...
        f"Modelo seleccionado con exactitud balanceada media en validación cruzada de {fmt_sig(row['cv_balanced_accuracy_mean'])} "
        f"(DE = {fmt_sig(row['cv_balanced_accuracy_std'], 2)}). En el conjunto retenido, obtuvo exactitud balanceada de "
        f"{fmt_sig(row['holdout_balanced_accuracy'])}, AUC de {fmt_sig(row['holdout_roc_auc'])}, sensibilidad para resistentes de "
        f"{fmt_sig(row['holdout_recall_resistant'])} y especificidad para susceptibles de {fmt_sig(row['holdout_specificity_susceptible'])}. "
        f"Los errores más delicados, es decir, resistentes predichos como susceptibles, fueron {int(row['holdout_false_susceptible'])}.\n\n"
...
(OUT / 'REPORTE_FINAL_NO_PRELIMINAR.md').write_text(report, encoding='utf-8')
...
with zipfile.ZipFile('ecoli_amr_resultados_finales.zip', 'w', compression=zipfile.ZIP_DEFLATED) as zf:
    for f in OUT.rglob('*'):
        zf.write(f, f.relative_to(Path('.')))

print('Análisis final completado.')
print('Salida:', OUT.resolve())
print('ZIP:', Path('ecoli_amr_resultados_finales.zip').resolve())
